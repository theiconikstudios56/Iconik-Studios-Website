...
export default function ProposalCTA({ proposal }: Props) {
  ...
  const [state, setState] = useState<ApprovalState>(alreadyApproved ? 'success' : 'idle');
          ...
          <StaggerContainer>
            ...
            <StaggerChild>
              <p style={{ fontSize: '16px', color: '#888', marginBottom: '8px', lineHeight: 1.6 }}>
                {alreadyApproved && proposal.approved_by_name
        ...
        {state !== 'success' && (
          <StaggerContainer>
            ...
            <StaggerChild>
              <h2 style={{
                fontFamily: "'Bebas Neue', sans-serif",
                fontSize: state === 'form' ? 'clamp(40px, 5vw, 64px)' : 'clamp(56px, 7vw, 96px)',
                lineHeight: 0.95,
                color: '#ffffff',
                letterSpacing: '0.04em',
                marginBottom: '16px',
            ...
            {/* Tier summary (shown when form opens) */}
            {state === 'form' && (
              <StaggerChild>
                <div style={{
                  display: 'inline-flex',
            ...
            {/* Approval form (form + loading states) */}
                ...
                <div style={{ maxWidth: '480px', marginBottom: '40px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    <button
                      ...
                      disabled={!name.trim() || !agreed || state === 'loading'}
                      style={{
                        ...
                        fontWeight: 700,
                        fontFamily: "'JetBrains Mono', monospace",
                        cursor: (!name.trim() || !agreed || state === 'loading') ? 'not-allowed' : 'pointer',
                        transition: 'all 0.2s',
                      }}
                    >
                      {state === 'loading' ? 'Confirming...' : 'Confirm & Approve →'}
                    </button>
                    {state !== 'loading' && (
                      <button
                        onClick={() => { setState('idle'); setName(''); setAgreed(false); setError(''); }}
                        style={{
